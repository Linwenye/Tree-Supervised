...
def train(epoch):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        #   for out4 classifier
        loss1 += criterion(outputs[0], labels)* (1 - args.loss_coefficient)
        loss1 += criterion(outputs[1], labels)* (1 - args.loss_coefficient)
        out1 = outputs[2].detach()
        out2 = outputs[3].detach()
        loss1 += kl_distill(outputs[0], out1) * args.loss_coefficient/2
        loss1 += kl_distill(outputs[0], out2) * args.loss_coefficient/2
        loss1 += kl_distill(outputs[1], out1) * args.loss_coefficient/2
        loss1 += kl_distill(outputs[1], out2) * args.loss_coefficient/2

        loss2 += criterion(outputs[2], labels)* (1 - args.loss_coefficient)
        loss2 += criterion(outputs[3], labels)* (1 - args.loss_coefficient)
        out3 = outputs[0].detach()
        out4 = outputs[1].detach()
        loss2 += kl_distill(outputs[2], out3) * args.loss_coefficient/2
        loss2 += kl_distill(outputs[2], out4) * args.loss_coefficient/2
        loss2 += kl_distill(outputs[3], out3) * args.loss_coefficient/2
        loss2 += kl_distill(outputs[3], out4) * args.loss_coefficient/2
