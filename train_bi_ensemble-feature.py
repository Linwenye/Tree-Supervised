...
parser.add_argument('--feature_loss_coefficient', default=0.003, type=float)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epoch):
        ...
        if epoch in [args.epoch // 3, args.epoch * 2 // 3, args.epoch - 15]:
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if init is False:
                ...
                for index in range(len(outputs_feature)):
            ...
            for j in range(len(outputs)):
                loss += torch.dist(net.adaptation_layers[j](outputs_feature[j]), teacher_feature) * \
        ...
        with torch.no_grad():
            ...
            for i in range(4):
                if correct[i] / total > best_single:
                    ...
                    torch.save(net.state_dict(), "./checkpoints/"+str(args.model)+".pth")
        print()
    print("Training Finished, TotalEPOCH=%d, Best Accuracy=%.4f, Best Single=%.4f" % (args.epoch, 100*best_acc, 100*best_single))
